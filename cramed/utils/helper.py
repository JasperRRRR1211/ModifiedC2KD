...
from utils.CramedDataset import CramedDataset
...
def pre_train_models(stu_type, tea_type, loader, epochs, learning_rate, device, args, save_model=False):
    ...
    if save_model:
        os.makedirs('./results', exist_ok=True)
        model_path_t = os.path.join('./results', 'teacher_mod_' + str(tea_type) + '_' + str(args.num_frame) + '_overlap.pkl')
        torch.save(tea_model.state_dict(), model_path_t)
        model_path_s = os.path.join('./results', 'student_mod_' + str(stu_type) + '_' + str(args.num_frame) + '_overlap.pkl')
...
def pre_train(stu_type, loader, epochs, learning_rate, device, args):
    tea_type = 1 - stu_type
    tea_model, stu_model = pre_train_models(stu_type, tea_type, loader, epochs, learning_rate, device, args, save_model=True)
